...
class MSCKF(object):
    ...
    def initialize_gravity_and_bias(self):
        # gyro bias from average angular velocity at rest
        ...
        # gravity in the IMU frame (opposite of accelerometer reading at rest)
        gravity_imu = sum_linear_acc / n

        # calibrate world-frame gravity magnitude to what this IMU measured
        gravity_norm = np.linalg.norm(gravity_imu)
        IMUState.gravity = np.array([0., 0., -gravity_norm])

        # initial orientation: q^I_G mapping -g_world -> gravity_imu
    ...
    def batch_imu_processing(self, time_bound):
        ...
        for msg in self.imu_msg_buffer:
            ...
            # skip stale messages (older than current state -> dt < 0)
        ...
        IMUState.next_id += 1
        self.imu_msg_buffer = self.imu_msg_buffer[used_msgs:]
    ...
    def process_model(self, time, m_gyro, m_acc):
        ...
        # continuous-time error-state dynamics (21x21)
        F = np.zeros((21, 21))
        F[:3, :3]    = -skew(gyro)
        ...
        # 3rd-order approx for the discrete transition matrix
        Fdt = F * dt
        Fdt2 = Fdt @ Fdt
        Fdt3 = Fdt2 @ Fdt
        Phi = np.identity(21) + Fdt + Fdt2/2.0 + Fdt3/6.0

        # propagate mean state with RK4
        self.predict_new_state(dt, gyro, acc)

        # OC-EKF modification (prevents spurious info gain on unobservable yaw)
        ...
        # update null-space states for next OC step
    ...
    def predict_new_state(self, dt, gyro, acc):
        ...
        # k1
        dq1 = 0.5 * Omega(gyro) @ q
        dv1 = to_rotation(q).T @ acc + g
        dp1 = v

        # k2
        q2  = q + dt/2 * dq1
        dq2 = 0.5 * Omega(gyro) @ q2
        dv2 = to_rotation(q2).T @ acc + g
        dp2 = v + dt/2 * dv1

        # k3
        q3  = q + dt/2 * dq2
        dq3 = 0.5 * Omega(gyro) @ q3
        dv3 = to_rotation(q3).T @ acc + g
        dp3 = v + dt/2 * dv2

        # k4
        q4  = q + dt * dq3
        dq4 = 0.5 * Omega(gyro) @ q4
        dv4 = to_rotation(q4).T @ acc + g
        dp4 = v + dt * dv3
        ...
        # timestamp is handled by batch_imu_processing, not here
    ...
    def state_augmentation(self, time):
        imu_state = self.state_server.imu_state
        R_imu = to_rotation(imu_state.orientation)
        R_imu_cam0 = imu_state.R_imu_cam0
        t_cam0_imu = imu_state.t_cam0_imu

        # camera pose in world frame
        R_cam = R_imu_cam0 @ R_imu
        p_cam = imu_state.position + R_imu.T @ t_cam0_imu

        # new CAMState
        new_cam_state = CAMState(imu_state.id)
        new_cam_state.timestamp = time
        new_cam_state.orientation = to_quaternion(R_cam)
        new_cam_state.position = p_cam
        ...
        # augmentation Jacobian. State ordering:
        #  [0:3]=theta_I, [3:6]=bg, [6:9]=v, [9:12]=ba,
        #  [12:15]=p_I, [15:18]=theta_extrinsic, [18:21]=p_extrinsic
        J = np.zeros((6, 21))
        J[:3, :3]    = R_imu_cam0
        J[:3, 15:18] = np.identity(3)
        J[3:6, :3]   = skew(R_imu.T @ t_cam0_imu)
        ...
        P_new[:old_size, :old_size] = self.state_server.state_cov
        P_new[old_size:, :old_size] = J @ self.state_server.state_cov[:21, :]
        P_new[:old_size, old_size:] = P_new[old_size:, :old_size].T
        P_new[old_size:, old_size:] = J @ self.state_server.state_cov[:21, :21] @ J.T

        self.state_server.state_cov = (P_new + P_new.T) / 2.0
    def add_feature_observations(self, feature_msg):
        ...
        self.tracking_rate = tracked_count / (num_current_features + 1e-5)
    ...
    def measurement_update(self, H, r):
        ...
        S = H @ P @ H.T + self.config.observation_noise * np.identity(len(H))
        ...
        dq = delta_x_imu[:3]
        dq_quat = np.array([*dq, 2.0]) / np.sqrt(4 + np.linalg.norm(dq)**2)
        imu_state.orientation = quaternion_multiplication(dq_quat, imu_state.orientation)
        ...
        for i, cam_state in enumerate(self.state_server.cam_states.values()):
            ...
            dq = delta_x_cam[:3]
            dq_quat = np.array([*dq, 2.0]) / np.sqrt(4 + np.linalg.norm(dq)**2)
            cam_state.orientation = quaternion_multiplication(dq_quat, cam_state.orientation)
            cam_state.position += delta_x_cam[3:6]

        # update covariance
        I = np.identity(len(self.state_server.state_cov))
        P_new = (I - K @ H) @ P
