...
def run_inference(model, test_root, seq_name, mean_path, std_path, device,
    ...
    if frame_gap == 1:
        ...
        delta_t_pred = delta_t_pred[::frame_gap]
        delta_q_pred = delta_q_pred[::frame_gap]
        n_pos = len(delta_t_pred) + 1
        gt_idx = np.arange(0, n_pos * frame_gap, frame_gap)
        gt_idx = gt_idx[gt_idx < cam["cam_positions"].shape[0]]
        delta_t_pred = delta_t_pred[:len(gt_idx) - 1]
        delta_q_pred = delta_q_pred[:len(gt_idx) - 1]
        pos_strided = cam["cam_positions"][gt_idx]
        q_strided   = cam["cam_quaternions"][gt_idx]
        ts_strided  = cam["cam_timestamps"][gt_idx]
...
def run_one(encoder_type, label, ckpt_path, test_root, seq, mean, std,
            out_dir, device, batch_size=32, frame_gap=1, use_evo=True):
...
def make_summary(all_metrics, labels, seqs, out_path):
    ...
    with open(out_path, "w") as f:
        f.write("# RMSE ATE V3 (m, lower is better) — dataset_100hz, V2 encoders\n\n")
        f.write(table + "\n")
...
def main():
    ...
    ap.add_argument("--out_dir",   default="./stage3_results_v3")
