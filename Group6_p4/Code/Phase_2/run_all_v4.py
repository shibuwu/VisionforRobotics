...
def build_model_v4(ckpt_path, encoder_type, device):
...
def build_model_v3(ckpt_path, encoder_type, device):
    ckpt = torch.load(ckpt_path, map_location=device, weights_only=False)
    cfg = ckpt.get("config", {})
    enc_kwargs = cfg.get("encoder_kwargs", {})
    encoder = ENCODER_REGISTRY[encoder_type](**enc_kwargs)
...
def run_inference(model, test_root, seq_name, mean_path, std_path, device,
    ...
    delta_t_pred = np.concatenate(all_t, 0)
    delta_q_pred = np.concatenate(all_q, 0)
    sign = np.where(delta_q_pred[:, :1] < 0, -1.0, 1.0)
    delta_q_pred = delta_q_pred * sign

    cam = np.load(Path(test_root) / seq_name / "camera.npz")
    n_pos = len(delta_t_pred) + 1
    pos_strided = cam["cam_positions"][:n_pos]
    q_strided   = cam["cam_quaternions"][:n_pos]
    ts_strided  = cam["cam_timestamps"][:n_pos]
...
def run_one(model_type, label, ckpt_path, test_root, seq, mean, std,
            out_dir, device, encoder_type, batch_size=32, frame_gap=1, use_evo=True):
    ...
    if deltas_path.exists():
        ...
        builder = build_model_v4 if model_type == "v4" else build_model_v3
        model, _ = builder(ckpt_path, encoder_type, device)
...
def make_summary(all_metrics, labels, seqs, out_path):
    ...
    with open(out_path, "w") as f:
        f.write("# RMSE ATE V4 (m, lower is better) — dataset_100hz, V2 encoders + homography aux loss\n\n")
        f.write(table + "\n")

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--runs_dir",  default="./runs")
    ap.add_argument("--test_root", default="./dataset_100hz/test")
    ap.add_argument("--mean",      default="./stats_v3/mean.npy")
    ap.add_argument("--std",       default="./stats_v3/std.npy")
    ap.add_argument("--out_dir",   default="./stage3_results_v4")
    ...
    networks = [
        ("inertial_v4", "inertial",  "v3", "runs/inertial_v3/best.pt"),
        ("vision_v4",   "vision_v2", "v4", "runs/vision_v4/best.pt"),
        ("fusion_v4",   "fusion_v2", "v4", "runs/fusion_v4/best.pt"),
    ]
    ...
    all_metrics = {}
    for label, encoder_type, model_type, ckpt_rel in networks:
        ckpt_path = Path(ckpt_rel)
        if not ckpt_path.exists():
            continue
        all_metrics[label] = {}
        for seq in seqs:
            m = run_one(model_type, label, ckpt_path, args.test_root, seq,
                        args.mean, args.std, out_dir, device, encoder_type,
    ...
    make_summary(all_metrics, [n[0] for n in networks], seqs, out_dir / "results_table.md")
